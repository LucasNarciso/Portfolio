...
function App () {
  ...
  //Função que muda o state quando o ponteiro do mouse fica sobre o Ícone
  const onOverIcon = (id) => {
    const icone1 = document.getElementById("1"+id);
    icone1.style.transform = "translateY(-80px)";

    const icone2 = document.getElementById("2"+id);
    icone2.style.transform = "translateY(60px)";
  }

  //Função que muda o state quando o ponteiro do mouse sai do Ícone
  const onLeaveIcon = (id) => {
    const icone1 = document.getElementById("1"+id);
    icone1.style.transform = "translateY(0em)";

    const icone2 = document.getElementById("2"+id);
    icone2.style.transform = "translateY(0em)";
  }

  //Função quie abre o link passado por parâmetro, em uma nova guia
  const abrirLink = (link) => {
    window.open(link, '_blank');
  }
  
  //Função que muda o scroll da página pelo id passado por parâmetro
  const scroll = (id) => {
    document.getElementById(id).scrollIntoView( {block: "start", inline: "nearest", behavior: "smooth"} );
  }
  
  //Função que renderiza os ícones de contato
  const renderIcones = icones.map((icon, index) => {
    if (1 === 1) {
      return(
        <Icones key={Math.random()} onMouseEnter={() => {onOverIcon("icone"+index)}} onClick={() => {abrirLink(icon.link)}} onMouseLeave={() => {onLeaveIcon("icone"+index)}}>{icon.svg}</Icones>
      )
    }
  });

  //Função que renderiza os detalhes dos ícones de contato
  const renderDetalhesIcones = icones.map((detIcon, index) => {

    return(
      <span key={"detalicones"+ index}>
        <DetalIcones >
          <p id={"1icone"+index} style={{transition: '0.2s ease-in-out', fontSize: '1.1em', fontWeight: 'bold'}}>{detIcon.nome}</p>
          <p id={"2icone"+index} style={{transition: '0.2s ease-in-out', fontSize: '0.9em', fontWeight: 'bold'}}>{detIcon.valor}</p>
        </DetalIcones>
      </span>
    )
  });

  //Função que renderiza os detalhes do projeto escolhido
  /*const renderDetalhesProjeto = detalhes.map((det, index) => {
    ...
    const renderImgs = (vetor) => {
        ...
        for (let i = 0; i < vetor.length; i++) {
          retorno[i] = <DivImgGaleria key={Math.random()} onClick={() => {definirModalIMG(vetor[i])}} style={{ backgroundImage: `url(${vetor[i]})` }}/>
        }
        }else{
    ...
    const renderLinks = (vetorLink, vetorNome) => {
        ...
        for (let i = 0; i < vetorLink.length; i++) {
            retorno[i] = <BotaoLinks key={Math.random()} onClick={() => {this.abrirLink(vetorLink[i])}}>{vetorNome[i]}</BotaoLinks>
        }
        }else{
    ...
    if (projetoSelecionado[index] === det.nome) {
  ...
  //Define qual vai ser a IMG dentro do modal, e chama ele
  const definirModalIMG = (img) => {
    console.log("Definiu Modal")
    setModalIMG(<Modal_ImagemAmpliada id={"ModalImgTelaCheia"} img={img} unset={() => {unsetModalIMG()}}/>)
  }

  //Retira o modal IMG de cena
  const unsetModalIMG = () => {
    setModalIMG(<></>)
  }

  const renderCardsProjetos = detalhes.map((projeto, index) => {
    return(
      <Card_Projeto imgs={[cardWhyZapIMG, cardWebDexIMG, cardLinkStackIMG]} index={index} key={Math.random()} definirModalIMG={definirModalIMG.bind(this)} abrirLink={abrirLink.bind(this)} detalhes={projeto}></Card_Projeto>
    )
  });
  ...
  return (
    <>
      ...
      <Container id={"inicio"}>
        ...
        <NavBar>
          <LinkNavBar onClick={() => {scroll("projetos")}}> PROJETOS</LinkNavBar>
          <LinkNavBar onClick={() => {scroll("inicio")}}> INÍCIO</LinkNavBar>
          <LinkNavBar onClick={() => {scroll("contatos")}}> CONTATOS</LinkNavBar>
        </NavBar>
        ...
        <DivFoto>
          ...
          <DivSetaDupla>
            <SVGSetaDupla onClick={() => {scroll("sobre")}} width="83" height="48" viewBox="0 0 83 48" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        <DivSobre id={'sobre'}>
          <Titulo>SOBRE</Titulo>
          <TextoSobre>
            <p>Meu nome é Lucas, atualmente tenho 21 anos e estou me formando em Análise e Desenvolvimento de Sistemas na Faminas de Muriaé - MG. Esse meu portfólio possui alguns dos meus projetos sobre Desenvolvimento Web, alguns pessoais, e outros são trabalhos passados pelos professores durante o curso.</p>
          </TextoSobre>
        </DivSobre>
        ...
        <DivProjetos id={"projetos"}>
          <Titulo>PROJETOS</Titulo>
            ...
            <DivCardsProjetos>
              ...
              {/* <CardProjeto>
                <CardTitulo> WhyZap </CardTitulo>
                <CardImagens src={cardWhyZapIMG} onClick={() => {setDetalheProjeto("WhyZap", 1)}}/>
                <DivDescProjetoFechada id={"DivDesc1"}>
                  {renderDetalhesProjeto}
                </DivDescProjetoFechada>
              </CardProjeto>

              <CardProjeto>
                <CardTitulo> WebDéx </CardTitulo>
                <CardImagens src={cardWebDexIMG} onClick={() => {setDetalheProjeto("WebDéx", 2)}}/>
                <DivDescProjetoFechada id={"DivDesc2"}>
                  {renderDetalhesProjeto}
                </DivDescProjetoFechada>
              </CardProjeto>

              <CardProjeto>
                <CardTitulo> LinkStack </CardTitulo>
                <CardImagens src={cardLinkStackIMG} onClick={() => {setDetalheProjeto("LinkStack", 3)}}/>
                <DivDescProjetoFechada id={"DivDesc3"}>
                  {renderDetalhesProjeto}
                </DivDescProjetoFechada>
              </CardProjeto> */}
              {renderCardsProjetos}
